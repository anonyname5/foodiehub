            ...
            if (file) {
                ...
                try {
                    ...
                    if (currentUser) {
                        ...
                        if (response.success) {
                            // Update current user data with new avatar path (relative path from DB)
                            // The response contains both 'url' (full URL) and 'path' (relative path)
                            // Store the path in the user object to match database format
                            currentUser.avatar = response.images[0].path || response.images[0].url;
                            window.api.setCurrentUser(currentUser);
                            
                            // Use the full URL for immediate display
                            const avatarUrl = response.images[0].url.startsWith('/storage/') 
                                ? `${window.location.origin}${response.images[0].url}`
...
function initializeUserAvatars() {
    ...
    if (currentUser && currentUser.avatar && 
        ...
        if (currentUser.avatar.startsWith('/storage/')) {
            avatarToUse = `${window.location.origin}${currentUser.avatar}`;
        } else if (currentUser.avatar.startsWith('storage/')) {
            avatarToUse = `${window.location.origin}/${currentUser.avatar}`;
        } else if (!currentUser.avatar.startsWith('http')) {
            // Relative path like "users/..." - convert to full URL
            avatarToUse = `${window.location.origin}/storage/${currentUser.avatar}`;
        } else {
            avatarToUse = currentUser.avatar;
        }

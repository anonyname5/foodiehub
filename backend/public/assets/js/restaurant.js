...
async function loadRestaurants() {
    try {
        ...
        // In monolith, restaurants are server-rendered - don't call API
        // Check if restaurants are already rendered in the page
        const restaurantsContainer = document.querySelector('#restaurants-container, #restaurants-grid');
        if (restaurantsContainer && restaurantsContainer.children.length > 0) {
            console.log('Restaurants already server-rendered, skipping API call');
            return;
        }
        
        // Only call API if no restaurants are rendered (shouldn't happen in monolith)
        console.warn('restaurant.js: Attempting to load restaurants via API - this should not happen in monolith');

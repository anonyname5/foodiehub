    ...
    <section class="panel">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            @foreach($days as $idx => $day)
                                @php
                                    $saved = old('hours.' . $day, $restaurant->hours[$day] ?? null);
                                @endphp
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="mb-2 flex gap-2">
                            <div class="flex-1">
                                ...
                                <input type="number" step="any" id="latitude" name="latitude" value="{{ old('latitude', $restaurant->latitude) }}" readonly
                            ...
                            <div class="flex-1">
                                ...
                                <input type="number" step="any" id="longitude" name="longitude" value="{{ old('longitude', $restaurant->longitude) }}" readonly
                                       class="w-full px-3 py-2 border rounded-lg bg-gray-50 text-gray-700">
                            </div>
        ...
        $imagesCollection = $restaurant->images instanceof \Illuminate\Support\Collection
            ...
            : $restaurant->images()->get();
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-images text-orange-500 mr-1"></i>Current Images
                        </label>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div class="relative border-2 border-gray-300 rounded-lg overflow-hidden">
                                <img src="{{ $imgUrl }}" alt="Restaurant Image" class="w-full h-32 object-cover">
                                @if(($image->is_primary ?? ($image['is_primary'] ?? false)))
                                <span class="absolute top-2 left-2 bg-orange-500 text-white px-2 py-1 rounded text-xs font-bold">Primary</span>
                                @endif
                                @if($imageId)
                                <form action="{{ route('images.destroy', $imageId) }}" method="POST" class="absolute top-2 right-2"
                                      data-confirm-title="Delete Image"
                                      data-confirm-message="Are you sure you want to delete this image?">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600">
                    ...
                    <div class="md:col-span-2">
                        <label for="images" class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-plus-circle text-orange-500 mr-1"></i>Add More Images
                        </label>
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-orange-400 transition">
                            ...
                            <div class="text-center">
                                <label for="images" class="cursor-pointer">
                                    ...
                                    <p class="text-sm text-gray-600 mb-1">Click to upload more images</p>
                        ...
                        <p class="text-xs text-gray-500 mt-2">
                            <i class="fas fa-info-circle mr-1"></i>New images will be added to existing ones. The first new image will be set as primary if no primary exists.
                        </p>
...
@php
    $debugAdminImages = ($imagesCollection ?? collect())->map(function($img) {
        return [
...
<script>
    ...
    try {
        const debugImages = @json($debugAdminImages);
        console.log('[ImageDebug] Admin edit images', debugImages);
    } catch (e) {
        console.warn('[ImageDebug] Failed to log admin images', e);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const lat = {{ old('latitude', $restaurant->latitude ?? 40.7128) }};
        const lng = {{ old('longitude', $restaurant->longitude ?? -74.0060) }};
